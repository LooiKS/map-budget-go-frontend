  ...
  async get() {
    ...
    for (var i = 0; i < trips.length; i++) {
      ...
      for (var j = 0; j < trips[i]["members"].length; j++) {
        trips[i]["members"][j] = await database.get(
          "users",
          trips[i]["members"][j]
        );
      ...
      for (var k = 0; k < trips[i]["schedules"].length; k++) {
        trips[i]["schedules"][k] = await database.get(
          "schedules",
          trips[i]["schedules"][k]
        );
        trips[i]["schedules"][k]["createdBy"] = await database.get(
          "users",
          trips[i]["schedules"][k]["createdBy"]
        );
        //    trips[i]["schedules"][k]["createdBy"]["friend"] = [];
      }
      for (var l = 0; l < trips[i]["expenses"].length; l++) {
        trips[i]["expenses"][l] = await database.get(
          "expenses",
          trips[i]["expenses"][l]
        );
        trips[i]["expenses"][l]["createdBy"] = await database.get(
          "users",
          trips[i]["expenses"][l]["createdBy"]
        );
        //    trips[i]["expenses"][l]["createdBy"]["friend"] = [];
        trips[i]["expenses"][l]["payBy"] = await database.get(
          "users",
          trips[i]["expenses"][l]["payBy"]
        );
        //    trips[i]["expenses"][l]["payBy"]["friend"] = [];
        for (var m = 0; m < trips[i]["expenses"][l]["sharedBy"].length; m++) {
          trips[i]["expenses"][l]["sharedBy"][m] = await database.get(
            "users",
            trips[i]["expenses"][l]["sharedBy"][m]
          );
